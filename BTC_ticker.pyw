...
# ── Config ────────────────────────────────────────────────────────────────────
UPDATE_MS      = 30_000      # 30 seconds
HISTORY_POINTS = 60          # number of points in the chart
GRAPH_W        = 90          # chart canvas width (px)
GRAPH_H        = 38          # chart canvas height (px)
WIN_H          = 42          # window height (≈ Windows taskbar clock)
BG             = "#000000"   # black background
FG_PRICE       = "#ffffff"   # price text color (white)
FG_LABEL       = "#000000"   # BTC label
COLOR_POS      = "#00e676"   # green gain
COLOR_NEG      = "#d50000"   # red loss
...
# ── APIs ──────────────────────────────────────────────────────────────────────
def fetch_binance():
    url = "https://api.binance.com/api/v3/ticker/24hr?symbol=BTCEUR"
...
class BTCTicker(tk.Tk):
    def __init__(self):
        ...
        self._setup_window()
        self._build_ui()
        self._schedule_update(delay=200)   # first fetch almost immediately
    ...
    def _build_ui(self):
        ...
        tk.Label(top_row, text="₿", font=("Consolas", 8, "bold"),
    ...
    def _fetch_and_refresh(self):
        price, change = fetch_btc()
        if price is not None:
            with lock:
                price_history.append(price)
            self.after(0, self._refresh_ui, price, change)
        self.after(0, self._schedule_update)   # reschedule
